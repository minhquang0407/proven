...
def main():
    ...
    if args.target in ("antigravity", "all"):
        destinations["Antigravity"] = user_home / ".gemini" / "config" / "skills" / "proven"
        # Also alias for softgnn-advisor backward compatibility
        destinations["Antigravity (Compat Alias)"] = user_home / ".gemini" / "config" / "skills" / "softgnn-advisor"
    if args.target in ("claude-code", "all"):
        destinations["Claude Code"] = user_home / ".claude" / "skills" / "proven"
    if args.target in ("cursor", "all"):
        destinations["Cursor / Local (.agents)"] = Path(".agents") / "skills" / "proven"

    print(f"[INSTALL] Installing Proven Skill from: {skill_source}")
