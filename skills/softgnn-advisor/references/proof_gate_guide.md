# SoftGNN Runtime Proof Gate Guide
...
SoftGNN enforces the **Runtime Proof Gate**:
...
When `verify_runtime_proof.py` (or `softgnn agent verify-proof`) is executed, it measures:
...
When a test fails the Proof Gate (0% lines executed or stopped at a guard clause), SoftGNN automatically returns a `self_healing` diagnosis block:
