...
class BranchDiagnoser:
    ...
    def diagnose_failure(
            ...
            if len(covered) == 0:
                ...
                if test_lines and not any(target_name in l for l in test_lines):
                    return {
                        ...
                        "target_id": target_id,
                        "explanation": f"Hàm '{target_name}' chưa từng được gọi trong file test '{test_file or 'test'}'.",
                        "actionable_suggestion": f"Hãy import và gọi trực tiếp '{target_name}(...)' với mock data hợp lệ trong test case.",
                    }
        ...
        return {
            ...
            "target_id": target_id,
            "explanation": f"Hàm '{target_name}' không được thực thi trong quá trình chạy test (0 dòng code được chạy).",
            "actionable_suggestion": f"Hãy kiểm tra xem test đã gọi trực tiếp '{target_name}' chưa, và đảm bảo các điều kiện guard clause ở đầu hàm được thoả mãn.",
        }
                ...
                if match:
                    ...
                    if target_name in mocked_target:
                        return {
                            ...
                            "explanation": (
                                f"Phát hiện hàm '{target_name}' bị mock ở dòng {idx} trong '{test_file}' "
                                f"(`{line.strip()}`). Khi bị mock, phần code logic thực tế không được chạy "
                                f"dẫn đến 0% Runtime Proof."
                            ),
                            "actionable_suggestion": (
                                f"Hãy bỏ mock hàm '{target_name}' hoặc sử dụng spy/integration test "
                                f"để code thực tế được kích hoạt khi chạy test."
                            ),
                        }
        return None
    ...
    def _diagnose_early_branch(
                ...
                for gpat in cls.GUARD_PATTERNS_MULTILANG:
                    ...
                    if m:
                        ...
                        return {
                            ...
                            "explanation": (
                                f"Hàm '{target_name}' bị rẽ nhánh sớm ở dòng {lno} do điều kiện `{line_str.strip()}`. "
                                f"Phần thân chính (dòng {missed_start}-{end_line}, gồm {missed_count} dòng) chưa được thực thi."
                            ),
                            "actionable_suggestion": (
                                f"Để kích hoạt sâu vào phần thân hàm (dòng {missed_start}-{end_line}), "
                                f"hãy truyền mock data hoặc tham số phủ định điều kiện thoát (`{cond}`)."
                            ),
                        }
        ...
        return {
            ...
            "missed_lines_range": [missed_start, end_line],
            "explanation": (
                f"Hàm '{target_name}' dừng thực thi ở dòng {last_line}. "
                f"Phần còn lại (dòng {missed_start}-{end_line}) chưa có bài test nào kích hoạt."
            ),
            "actionable_suggestion": f"Hãy bổ sung test case với dữ liệu phù hợp để luồng thực thi đi tiếp qua dòng {last_line}.",
        }
    ...
    def _inspect_python_ast_branch(
        ...
        if matched_if:
            ...
            action_hint = (
                f"Để kích hoạt sâu vào phần thân hàm (dòng {missed_start}-{end_line}), "
                f"hãy bổ sung test case truyền tham số hoặc mock data sao cho điều kiện `{cond_str}` là False (hoặc hợp lệ)."
            )
            ...
            return {
                ...
                "missed_lines_count": missed_count,
                "explanation": (
                    f"Hàm bị rẽ nhánh sớm ở dòng {branch_line} do điều kiện `{branch_code}`. "
                    f"Phần thân chính (dòng {missed_start}-{end_line}) chưa được kích hoạt."
                ),
                    ...
                    if calls_target and call_lineno:
                        ...
                        if fn_covered and max(fn_covered) < call_lineno:
                            ...
                            return {
                                ...
                                "explanation": (
                                    f"Hàm '{target_name}' không được gọi vì hàm cấp trên ('{node.name}' trong '{file_rel}') "
                                    f"bị rẽ nhánh sớm ở dòng {branch_line} (`{branch_code}`) trước khi đến lời gọi ở dòng {call_lineno}."
                                ),
                                "actionable_suggestion": (
                                    f"Hãy truyền tham số/mock data cho '{node.name}' để vượt qua điều kiện dòng {last_caller_line}."
                                ),
                            }
        return None
                ...
                for gpat in cls.GUARD_PATTERNS_MULTILANG:
                    ...
                    if m:
                        ...
                        return {
                            ...
                            "condition": cond,
                            "explanation": f"Hàm '{target_name}' có điều kiện kiểm tra đầu vào ở dòng {lno}: `{line.strip()}`.",
                            "actionable_suggestion": f"Khi viết test, hãy đảm bảo truyền tham số hoặc mock data thoả mãn điều kiện `{cond}` để không bị thoát sớm.",
                        }
        return None
